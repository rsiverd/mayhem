...
Program options:
    ...
    -B, --before=N      include N days before fdate in stack [def: $days_before]
   ...
   case $1 in
     ...
     #   ;;
      ...
      -A|--after)
         case $2 in
            -*)
               msg="Option -A|--after needs a positive integer argument!"
               Recho "\n${msg}\n" >&2
               usage >&2
               exit 1
               ;;
      ...
      -B|--before)
         case $2 in
            -*)
               msg="Option -B|--before needs a positive integer argument!"
               Recho "\n${msg}\n" >&2
               usage >&2
               exit 1
               ;;
            *)
               if !( int_check_pass $2 ) || [ $2 -lt 0 ]; then
                  Recho "Invalid days_before: " >&2 ; Yecho "$2 \n\n" >&2
                  exit 1
               fi
               days_before=$2
               ;;
         esac
         [ $vlevel -ge 0 ] && yecho "Stack days before: ${days_before}\n" >&2
         shift 2
         ;;
      #-------------------------------------------------------------------
      -A|--after)
...
pick_bias () {
   local image="$1"
   local ibase="${image##*/}"
   local bdate=$(echo $ibase | cut -d- -f3)
   echo $save_root/$camid/$bdate/med_bias_${bdate}_${drtag}.fits
}
...
if [ -f $nite_dark ]; then
   ...
   for image in "${dark_list[@]}"; do
      ...
      echo "ibase: $ibase"
      echo "fdate: $fdate"
      use_bias=`pick_bias $image`
      if [ ! -f $use_bias ]; then
         recho "Bias missing: '$use_bias'\n"
         exit 1
      fi
...
#     -- days_before and days_after are now selectable on the command line
