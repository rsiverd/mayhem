...
barrier_check_pass () {
   ...
   matches=0
   #echo "nites: ${nites[*]}"
   [ -f $barrier_list ] || return
   awk -v camid=$camid -v otype=$otype '{
      if (($1 == camid) && ($2 == otype)) print $3,$4
   }' $barrier_list | while read fdate1 fdate2; do
      echo "fdate1: $fdate1"
      echo "fdate2: $fdate2"
      echo "nites: ${nites[*]}"
      if ( dates_are_listed $fdate1 $fdate2 ${nites[*]} ); then
         echo "BOTH DATES LISTED!!"
         return 1
      fi
      echo ===========================
   done
